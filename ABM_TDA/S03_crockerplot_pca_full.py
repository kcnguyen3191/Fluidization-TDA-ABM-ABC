...
for irun in range(num_run):
    print(irun)
    b0_end_embedded  = PCA(n_components=3).fit_transform(b0_end_mat)
    b1_end_embedded  = PCA(n_components=3).fit_transform(b1_end_mat)
    b01_end_embedded = PCA(n_components=3).fit_transform(b01_end_mat)
    ...
    np.save('./Simulated_Grid/ODE/pca_results_full'+str(irun+1)+'_angles.npy',results)
print("Job done!")


tsne_results = np.load('./Simulated_Grid/ODE/pca_results_full'+str(0+1)+'_angles.npy',allow_pickle=True).item()
...
fig.tight_layout()
fig.savefig('./Simulated_Grid/ODE/pca_crocker_heatmap_angles.pdf',bbox_inches='tight')
plt.show()
...
scipy.io.savemat('./Simulated_Grid/ODE/pca_mat_angles_SD.mat',mat_tsne)
