...
class Dorsogna:
    def __init__(self,sigma=0.00224,alpha=10/1000,beta=100/1000,
                 cA=2/1000,cR=4/1000,lA=12/1000,lR=6/1000,
...
#         print(lR)
    ...
    def diff(self,t,Z):
        ...
        y = np.squeeze(y)
        if 'left' in self.BCs:
            vx[x<0.0] = np.abs(vx[x<0.0])
        if 'right' in self.BCs:
            vx[x>100.0] = -np.abs(vx[x>100.0])
        if 'top' in self.BCs:
            vy[y<0.0] = np.abs(vy[y<0.0])
        if 'bottom' in self.BCs:
            vy[y>100.0] = -np.abs(vy[y>100.0])
