...
def compute_crocker_custom(traj_df,frame_vec,prox_vec,
                    data_cols=('x','y','vx','vy'),
                    betti=[0,1]):
    '''Compute crockers for specific Betti numbers given a trajectory dataframe.
...
def plot_crocker_highres(crocker,prox_vec,frame_list=None,
                 save_path=None):
...
def plot_crocker_highres_split(crocker,prox_vec,frame_list,crocker1,prox_vec1,frame_list1,
                 save_path=None):
    fig,(ax1,ax2) = plt.subplots(1,2,sharex=True,sharey=True,figsize=(12,8),dpi=400)
    bcs0 = ax1.contourf(*np.meshgrid(range(1,crocker.shape[0]+1),
                                     prox_vec,indexing='ij'),
    ...
    cbar1 = fig.colorbar(bcs0,ax=ax1)
    ...
    plt.setp(ax1, xticks=[10, 30, 50], xticklabels=frame_list)
    ax2.set_xlabel('Time (frame)',fontsize=30)
    ax2.tick_params(axis='x', labelsize=20)
    ax2.tick_params(axis='y', labelsize=20)
    plt.setp(ax2, xticks=[10, 30, 50], xticklabels=frame_list)
    plt.tight_layout()
    if save_path == None:
        return fig
    else:
